...
- O pool PostgreSQL possui capacidade e timeouts explícitos: `DB_POOL_MAX`, `DB_CONNECTION_TIMEOUT_MS`, `DB_IDLE_TIMEOUT_MS` e `DB_STATEMENT_TIMEOUT_MS`. Os templates usam valores maiores em desenvolvimento e conservadores para tiers pequenos de Supabase/Render; ajuste `DB_POOL_MAX` considerando todas as instâncias da API.
