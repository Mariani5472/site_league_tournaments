...
export function MatchVoting({ matchId, canResolve }: { matchId: string; canResolve: boolean }) {
  ...
  if (match.isLoading) return <p className="text-muted-foreground">Carregando partida...</p>; if (match.isError || !match.data) return <p className="text-destructive" role="alert">Não foi possível carregar a partida.</p>;
