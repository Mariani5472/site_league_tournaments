...
export function TeamSelection({ lobby }: {
    ...
    if (selection.completed)
        return <section className="rounded-2xl border border-emerald-500/30 bg-emerald-500/5 p-5"><h2 className="font-semibold text-emerald-700">Times definidos</h2><p className="mt-1 text-sm text-muted-foreground">Modo: {options.find(option => option.mode === selection.mode)?.title}. Aceitar os times não confirma sua prontidão.</p><p className="mt-2 font-medium">{lobby.readyCount}/{lobby.playersCount} jogadores prontos. Use o botão “Confirmar prontidão” abaixo.</p></section>;
