...
export async function draftPick(leagueId: string, lobbyId: string, userId: string) {
  return (await api.post<LobbyDetails>(`/leagues/${leagueId}/lobbies/${lobbyId}/team-selection/pick`, { user_id: userId })).data;
}
...
export async function voteCaptain(leagueId: string, lobbyId: string, candidateId: string) {
  return (await api.post<LobbyDetails>(`/leagues/${leagueId}/lobbies/${lobbyId}/team-selection/captain-vote`, { candidate_id: candidateId })).data;
}
